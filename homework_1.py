...
for i in range(
        len(sorted_rand_numbers)):  # create the locator which will tell which position in the list we are currently
    for x in range(i + 1, len(sorted_rand_numbers)):
        # compare the value at the current position with all values after it in the list.
        if sorted_rand_numbers[i] > sorted_rand_numbers[x]:
            # If the value at the current position is greater than the value at a next position, than
            sorted_rand_numbers[i], sorted_rand_numbers[x] = sorted_rand_numbers[x], sorted_rand_numbers[i]
            # the two values are swapped using tuple unpacking
...
# create variable with default value of 0 which will be changed afterwards and hold the average value for even nums
average_even = 0
# create variable with default value of 0 which will be changed afterwards and hold the average value for odd nums
average_odd = 0
# iterating through generated numbers and if the remainder of the number equals to 0, it is even num, if not than odd
