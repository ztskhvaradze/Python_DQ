...
def sort_list(input_list):
    ...
    for i in range(len(input_list)):  # create the locator which will tell which position in the list we are currently
        ...
        for x in range(i + 1, len(input_list)):
            ...
            if input_list[i] > input_list[x]:
                input_list[i], input_list[x] = input_list[x], input_list[
                    i]  # the two values are swapped using tuple unpacking
    return input_list
...
def get_average_even_odd(input_lst):
    ...
    # changing the default value of variables with average value (rounded to 2 decimal points for better visual)
